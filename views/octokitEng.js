...
const firstPartKey =
  "github_pat_11A3VST7I0FwNmyd4Z58Hl_4SLIQBSZSl0AUu2fZMdRNfY4bBF22MY";
const secondPartKey = "ZieOTVV02ffQJJGTJWENNNEENX6E";
...
const addToWordArr = (response, array) => {
  ...
  console.log(uncodedRepository);
  //change link
  let fixedText = uncodedRepository.replaceAll(
    "![[pzn-apps/img",
    "![alt text](https://github.com/pzn-apps/pzn-apps.github.io/blob/main/img"
  );
...
const createSubFolder = (arr, index, subacontent) => {
  ...
  pathWordDocument = arr.map((item) =>
    item.replace(/ /g, "").replace(/.md/g, "").replace(/'/g, "")
  );
...
projectHeader.map((item, index) =>
  item.addEventListener("click", () => {
    ...
    if (index === 0)
      createSubFolder(wordAutoFillFolders, index, wordAutoFillContent);
